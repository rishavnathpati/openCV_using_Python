...
def line_detection_non_vectorized(image, edge_image, num_rhos=180, num_thetas=180, t_count=300):
...
if __name__ == "__main__":
    ...
    image = cv2.imread(path+"five_shapes.png")
    edge_image = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    # edge_image = cv2.GaussianBlur(edge_image, (3, 3), 1)
    edge_image = cv2.Canny(edge_image, 20, 20)
